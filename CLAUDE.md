...
factions.html     Factions Overview (nav: FACTIONS) — 4 faction cards + featured panel
...
design/           10 original design reference PNGs (1440×1024) — the source of truth
...
**2026-08-02 (nav / snap fix):** The home page snapped down 92px on load and hid the nav. Cause: `.hero` carried `scroll-snap-align: start`, but the hero begins *below* the 92px nav, so the browser snapped the hero's top to the viewport top the moment the page rendered. **The hero is no longer a snap target** (`css/home.css`) — only `body > section.section` is. With no snap point near the top, the page rests at scrollY 0 with the nav visible, and the hero scrolls freely, so the backdrop parallax actually plays instead of being yanked back (measured drift 40 / 80 / 140px at scrollY 200 / 400 / 700; previously any scroll under ~500px was pulled straight back to the top snap point). Section snapping is unchanged. **Don't re-add a snap point at the top of this page** — anchoring it to the hero hides the nav, and anchoring it to the nav restores the yank-back.

**2026-08-02 (factions fan):** The four Overview cards are now a **fanned 3-D deck** instead of a flat row (`css/factions.css`): `.fo-cards` is a flex row with `perspective: 1800px`, each card `rotateY(16deg)` off a `transform-origin: left center` and pulled `margin-left: -26px` so they overlap; `z-index` runs 4→1 left-to-right with a `brightness()` falloff (0.9 / 0.82 / 0.74) for depth. The active card sits at `rotateY(7deg) translateZ(46px)`, hover at `rotateY(3deg) translateZ(80px) translateY(-8px)`. Because the cards overlap, the card body/number/emblem are inset `36px` from the left so no name is hidden behind its neighbour — **if you change the overlap, re-check that inset.** Below 1180px the fan collapses to a flat 2×2 grid (transforms and negative margins reset). Card art swapped to match the dossiers each card opens: Enforcer → `fc-police.webp` (cropped from `op-hero-poster.jpg`), and **The Seekers was renamed "Hackers"** with `fc-hackers.webp` (cropped from `hacker-hero.webp`), the Phantom Protocol emblem, and copy rewritten to match that page — the design's mystical "Data mystics / ASCENSION" wording no longer fitted. `fc-enforcer.webp` and `fc-seekers.webp` were deleted. Outcasts still has no dossier.

**2026-07-31 (factions overview):** `factions.html` was rebuilt from the `Factions_Overview.md` Diffui build (`authToken=51p9t5EwEhE3`), replacing the hand-composed hub with the designed page: full-bleed banner-lit city backdrop, the FACTIONS headline, four chamfered faction cards (Syndicate / Enforcer / The Seekers / Outcasts, each with its own accent, emblem, portrait and number badge) and the cyan featured-faction panel. Six assets generated, five kept (2 high with design-reference conditioning, 3 medium card portraits). The Syndicate card and the featured panel deliberately share one image (`fo-featured.webp`) — as in the design, they are the same character; the separately generated `fc-syndicate.webp` was dropped. **The design names four factions but only three dossiers exist**, so cards map: Syndicate → `character.html`, Enforcer → `operative.html`, The Seekers → `hackers.html` (nearest match — a knowledge/data faction, but *not* the same name as Phantom Protocol), Outcasts → no page yet, rendered in a muted `.is-pending` state with no link. Both mismatches need Zack's call.
